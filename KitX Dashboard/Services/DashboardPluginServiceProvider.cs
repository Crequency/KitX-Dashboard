...
// ─────────────────────────────────────────────────────────────────────────────
// DashboardPluginServiceProvider — bridges the Dashboard's live plugin services
...
// (KitX.Core/Plugin/PluginsManager.cs), which is the Dashboard's native plugin-call
// path — the lookup-by-name, connector request, and response correlation all line up.
//
// Registered as a singleton in App.axaml.cs alongside RealPluginManager.
// ─────────────────────────────────────────────────────────────────────────────

/// <summary>
/// Adapts the Dashboard's <see cref="IPluginServer"/> and <see cref="IEventService"/>
...
public sealed class DashboardPluginServiceProvider : IPluginServiceProvider
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// Subscribes to the Dashboard's <c>PluginResponse</c> event channel and forwards
